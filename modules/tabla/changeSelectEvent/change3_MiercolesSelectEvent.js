...
export { changeMiercolesSelectEvent };
...
// Definimos la función changeMiercolesSelectEvent que se ejecuta cuando se cambia el valor de un elemento select
function changeMiercolesSelectEvent(tabla, database, collection) {
  ...
  tabla.addEventListener("change", function (event) {
    ...
    if (event.target.classList.contains("miercoles-select")) {
      // Preguntamos al usuario si está seguro de cambiar el estado
      const confirmar = confirm("¿Estás seguro de que deseas cambiar el monto de pago del día Miercoles?");
      if (confirmar) {
        // Obtenemos el ID del elemento a partir del atributo data-id del select
        const id = event.target.getAttribute("data-id");
        // Obtenemos el nuevo estado seleccionado
        const nuevoMiercoles = event.target.value;

        // Actualizamos el estado en la base de datos en la ruta especificada
        update(ref(database, `${collection}/${id}`), { miercoles: nuevoMiercoles })
          .then(() => {
